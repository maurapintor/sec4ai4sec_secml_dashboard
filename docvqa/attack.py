"""Pix2Struct adversarial-forgery attack. Verbatim from pralab/adv-docVQA
(attacks/pix2struct_attack.py), adapted to this project's vendored modules.
"""
...
from secmlt.optimization.constraints import L2Constraint, LInfConstraint, MaskConstraint
from secmlt.optimization.gradient_processing import LinearProjectionGradientProcessing
from secmlt.optimization.initializer import Initializer
from torch.utils.data import DataLoader, TensorDataset
from transformers.image_utils import to_numpy_array

from docvqa.constraints import QuantizationConstraintWithMask
from docvqa.masks import mask_include_all
from docvqa.modular_attack import ModularEvasionAttackFixedEps
...
class Pix2StructAttack(ModularEvasionAttackFixedEps):
    """Attack implementing a custom (multi-question, gradient-accumulating) forward for Pix2Struct."""

    def __init__(self, processor: Pix2StructImageProcessor, questions: Union[str, List[str]], *args, **kwargs):
...
def questions_target_validation(questions, targets):
    assert (
        (isinstance(questions, str) and isinstance(targets, str))
        or (isinstance(questions, list) and isinstance(targets, list) and len(questions) == len(targets))
        or ((isinstance(questions, list) or isinstance(questions, str)) and targets is None)
...
def attack_pix2struct(
    ...
    """Compute adversarial perturbation for Pix2Struct model. Returns the adversarial PIL image."""
    ...
    norm_constraint = L2Constraint if perturbation_model == "l2" else LInfConstraint
    lp_model = LpPerturbationModels.L2 if perturbation_model == "l2" else LpPerturbationModels.LINF

    perturbation_constraints = [
        MaskConstraint(mask=perturbation_mask),
        norm_constraint(radius=float(eps)),
    ]
    domain_constraints = [
    ...
    gradient_processing = LinearProjectionGradientProcessing(lp_model)
